...
public class LectureSqlProvider {
    public String list(Map paramMap){
        ...
        if (null != latitude && null != longitude){
            sql.append("(power(t.latitude-#{latitude}, 2)+power(t.longitude-#{longitude}, 2)) s, ");
        }
        ...
        sql.append(" from lecture t where id >  t.is_deleted = 0 and t.start_time > now()");
        if (null != latitude && null != longitude){
            sql.append(" order by s desc limit #{pageSize}");
        }
        return sql.toString();
    }
}
